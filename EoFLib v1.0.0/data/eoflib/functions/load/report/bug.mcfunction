#region
	# Uh... Ricardo Stuff
execute if entity @s[advancements={eoflib:ricardo_report=true}] run tellraw @s [{"text": "\nHere you go!", "color": "gray", "hoverEvent": {"action": "show_text", "contents": {"text": "Say bye to Ricardo!", "color": "gray"}}, "clickEvent": {"action": "run_command", "value": "/trigger eoflib.trigger set 10"}}]
execute if entity @s[advancements={eoflib:ricardo_report=true}] run scoreboard players reset @s eoflib.dropped_ricardo
execute if entity @s[advancements={eoflib:ricardo_report=true}] anchored eyes positioned ^ ^ ^ if entity @e[type=item, limit=1, sort=nearest, distance=..0.4, nbt={Item: {tag: {eoflib: {}}}}] run function eoflib:loop/time/10t/kill_items
